...
class _StatistiquesScreenState extends State<StatistiquesScreen> with SingleTickerProviderStateMixin {
    ...
    return Scaffold(
      ...
      body: Consumer<DatabaseProvider>(
        builder: (context, provider, child) {
          ...
          final annees = statsParAnnee.keys..sort((a, b) => b.compareTo(a));
  ...
  Widget _buildEvolutionRendementChart(Map<int, Map<String, dynamic>> statsParAnnee) {
    final annees = statsParAnnee.keys..sort();
    if (annees.isEmpty) {
      return Card(
        child: Padding(
          padding: const EdgeInsets.all(16),
                ...
                BarChartData(
                  ...
                  barGroups: rendements.entries.map((entry) {
                    return BarChartGroupData(
                      x: rendements.keys.indexOf(entry.key),
                      barRods: [
                        BarChartRodData(
                          toY: entry.value,
                          color: Colors.orange,
  ...
  Widget _buildEvolutionRendementParcelleChart(List<Chargement> chargements, int parcelleId, List<Parcelle> parcelles) {
    ...
    final chargementsParcelle = chargements.where((c) => c.parcelleId == parcelleId);
    ...
    final annees = rendementsParAnnee.keys..sort();
    if (annees.isEmpty) {
      return Card(
        child: Padding(
          padding: const EdgeInsets.all(16),
  ...
  Widget _buildEvolutionHumiditeParcelleChart(List<Chargement> chargements, int parcelleId, List<Parcelle> parcelles) {
    ...
    final annees = humiditeParAnnee.keys..sort();
    if (annees.isEmpty) {
      return Card(
        child: Padding(
          padding: const EdgeInsets.all(16),
          ...
          child: BarChart(
            BarChartData(
              ...
              barGroups: rendementsMoyens.entries.map((entry) {
                return BarChartGroupData(
                  x: rendementsMoyens.keys.indexOf(entry.key),
                  barRods: [
                    BarChartRodData(
                      toY: entry.value,
                      color: Colors.orange,
  ...
  Widget _buildRepartitionSurfacesVarietesChart(List<Semis> semis, List<Parcelle> parcelles) {
    ...
    final semisAnnee = semis.where((s) => s.date.year == anneeSelectionnee);
