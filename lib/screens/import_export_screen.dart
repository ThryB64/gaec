...
class ImportExportScreen extends StatelessWidget {
  ...
  Future<void> importData(BuildContext context) async {
    ...
    for (var p in (data['parcelles'] as List)) {
      await provider.addParcelle(Parcelle.fromMap(Map<String, dynamic>.from(p)));
    }
    for (var c in (data['cellules'] as List)) {
      await provider.addCellule(Cellule.fromMap(Map<String, dynamic>.from(c)));
    }
    for (var ch in (data['chargements'] as List)) {
      await provider.addChargement(Chargement.fromMap(Map<String, dynamic>.from(ch)));
    }
    for (var s in (data['semis'] as List)) {
      await provider.addSemis(Semis.fromMap(Map<String, dynamic>.from(s)));
    }
    for (var v in (data['varietes'] as List)) {
      await provider.addVariete(Variete.fromMap(Map<String, dynamic>.from(v)));
    }
