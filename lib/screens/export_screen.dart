...
class _ExportScreenState extends State<ExportScreen> {
  ...
  Future<void> _generatePDF() async {
    try {
      ...
      await db.loadChargements();
      await db.loadParcelles();
      await db.loadCellules();
      await db.loadSemis.fromMap();
      final chargements = db.chargements;
      final parcelles = db.parcelles;
      final cellules = db.cellules;
      final semis = db.semis;
      final chargementsAnnee = chargements.where((c) => c.dateChargement.year == _selectedYear);
      ...
      for (var parcelle in parcelles) {
        final chargementsP = chargementsAnnee.where((c) => c.parcelleId == parcelle.id);
      ...
      for (var parcelle in parcelles) {
        ...
        for (var i = 0; i < chargementsP.length; i += 25) {
          final pageChargements = chargementsP.skip(i).take(25);
                      ...
                      ...parcelles.where((p) {
                        ...
                        return pw.TableRow(
                          ...
                          children: [
                            ...
                            _buildDataCell(db.getVarieteForParcelle.fromMap(p.id)?.nom ?? "Inconnue"),
                ...
                pw.Table(
                  ...
                  children: [
                    ...
                    ...varietesMap.entries.map((entry) {
                      ...
                      return pw.TableRow(
                        decoration: pw.BoxDecoration(
                          color: varietesMap.keys.indexOf(variete) % 2 == 0 
                              ? PdfColors.white 
                              : PdfColors.grey200,
                        ),
                        children: [
